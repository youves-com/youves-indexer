...
async def on_quipu_add_liquidity(
    _ctx: HandlerContext,
    add_liquidity,
) -> None:
    ...
    await models.Trade.create(
        ...
        symbol=symbol,
        lp_token_total_supply=Decimal(add_liquidity.storage.storage.total_supply),
        created=add_liquidity.data.timestamp,
        token_1_pool=Decimal(add_liquidity.storage.storage.tez_pool),
        token_2_pool=Decimal(add_liquidity.storage.storage.token_pool),
    )
